...
for (const product of search) {
    test(`verify product header ${product.productName}`, { tag: ['@product,@sanity,@regression'] }, async ({ homePage }) => {
        ...
        const productInfo: ProductInfoPage = await resultsPage.selectTheProduct(product.productName);
...
test('verify product metadata', async ({ homePage }) => {
    ...
    const productInfo: ProductInfoPage = await resultsPage.selectTheProduct('MacBook Pro');
    await homePage.page.waitForTimeout(2000);
